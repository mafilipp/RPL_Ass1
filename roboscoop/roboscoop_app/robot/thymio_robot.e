...
feature {NONE} -- Initialization
	...
	make
		...
		do
			-- Initialize sensors.
			create range_sensors.make_with_topic (separate_str ({THYMIO_TOPICS}.prox_horizontal))
			create ground_sensors.make_with_topic (separate_str ({THYMIO_TOPICS}.prox_ground))
			create odometry_signaler.make_with_topic (separate_str ({THYMIO_TOPICS}.odometry))

			-- Initialize actuators.
			create diff_drive.make_with_topic (separate_str ({THYMIO_TOPICS}.velocity))
			create sound_player.make_with_topic (separate_str ({THYMIO_TOPICS}.sound))
			create top_leds.make_with_topic (separate_str ({THYMIO_TOPICS}.top_leds))
			create buttons_leds.make_with_topic (separate_str ({THYMIO_TOPICS}.buttons_leds))
			create circle_leds.make_with_topic (separate_str ({THYMIO_TOPICS}.circle_leds))
