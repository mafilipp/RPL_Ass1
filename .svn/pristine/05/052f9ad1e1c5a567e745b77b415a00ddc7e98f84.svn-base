...
feature {WANDER_BEHAVIOUR} -- Control
	...
	go (d_sig: separate WANDER_SIGNALER; o_sig: separate ODOMETRY_SIGNALER; r_sig: separate RANGE_SENSOR_GROUP;
		g_sig: separate GROUND_SENSOR_GROUP; s_sig: separate STOP_SIGNALER;
	...
	stop_when_obstacle_huge (d_sig: separate WANDER_SIGNALER;
								o_sig: separate ODOMETRY_SIGNALER; r_sig: separate RANGE_SENSOR_GROUP;
								g_sig: separate GROUND_SENSOR_GROUP; s_sig: separate STOP_SIGNALER;
								drive: separate DIFFERENTIAL_DRIVE; player: separate SOUND_PLAYER)
		...
		do
			if s_sig.is_stop_requested then
				...
				if r_sig.is_obstacle_in_front then
					if r_sig.is_obstacle_at_left then
						vxsign := 1
						vthetasign := 1
					elseif r_sig.is_obstacle_at_right then
						vxsign := 1
						vthetasign := -1
					end
